...
const GSAPSplitText = null;

gsap.registerPlugin(ScrollTrigger, useGSAP); // We will conditionally try to register GSAPSplitText if it exists
...
const Shuffle = ({
    ...
    useEffect(() => {
        ...
        // Attempt registration silently if we managed to import it
        if (GSAPSplitText) { gsap.registerPlugin(GSAPSplitText); }
    }, []);
        ...
        () => {
            ...
            const build = () => {
                ...
                try {
                    ...
                    console.warn("GSAPSplitText failed, likely missing module:", e);
                    // Minimal fallback: split text by basic span wrapper to avoid crashing the whole page if missing.
                    el.innerHTML = text.split('').map(c => `<div style="display:inline-block;" class="shuffle-char">${c === ' ' ? '&nbsp;' : c}</div>`).join('');
                    splitRef.current = { chars: Array.from(el.querySelectorAll('.shuffle-char')) };
                }
