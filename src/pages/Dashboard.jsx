...
const Dashboard = () => {
    ...
    const handleFileUpload = async (event) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                alert("Audio uploaded! Processing started.");
    ...
    const GlobalPerformance = () => (
        <>
            <div className="header-row">
                <h1>Global Performance & Recordings</h1>
            ...
            <>
                <div className="header-row">
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        <h1>{selectedUser.name}'s Profile</h1>
                    </div>
                </div>
                                ...
                                {recordings.map((rec) => (
                                    <div className="activity-item" key={rec._id} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', borderBottom: '1px solid #eee' }}>
                                        <div>
                                            ...
                                            <p style={{ margin: 0, color: '#666', fontSize: '0.9rem' }}>Status: {rec.status} | Duration: {formatTime(rec.duration)}</p>
                                        </div>
                                        <div>
